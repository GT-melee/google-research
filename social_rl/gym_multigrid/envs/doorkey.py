...
class DoorKeyEnv(multigrid.MultiGridEnv):
    ...
    def __init__(self, size=8, n_agents=3, **kwargs):
        super().__init__(grid_size=size, max_steps=10 * size * size, n_agents=n_agents, **kwargs)
    ...
    def _gen_grid(self, width, height):
        ...
        self.put_obj(multigrid.Door("yellow", is_locked=True), self.split_idx, door_idx)

        # Place a yellow key on the left side
        self.place_obj(obj=minigrid.Key("yellow"), top=(0, 0), size=(self.split_idx, height))
...
register(env_id="MultiGrid-DoorKey-6x6-v0", entry_point=module_path + ":DoorKeyEnv6x6")

register(env_id="MultiGrid-DoorKey-8x8-v0", entry_point=module_path + ":DoorKeyEnv")

register(env_id="MultiGrid-DoorKey-16x16-v0", entry_point=module_path + ":DoorKeyEnv16x16")

register(env_id="MultiGrid-DoorKey-5x5-Single-v0", entry_point=module_path + ":DoorKeyEnv5x5Single")

register(env_id="MultiGrid-DoorKey-6x6-Single-v0", entry_point=module_path + ":DoorKeyEnv6x6Single")

register(env_id="MultiGrid-DoorKey-8x8-Single-v0", entry_point=module_path + ":DoorKeyEnv8x8Single")

register(env_id="MultiGrid-DoorKey-16x16-Single-v0", entry_point=module_path + ":DoorKeyEnv16x16Single")
