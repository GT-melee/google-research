...
class CoinGameEnv(multigrid.MultiGridEnv):
    ...
    def __init__(self, size=15, n_agents=2, n_goals=3, n_clutter=0, n_colors=3, max_steps=20, **kwargs):
    ...
    """
        ...
        if self.minigrid_mode:
            ...
            self.position_obs_space = gym.spaces.Box(
                low=0, high=max(size, n_colors), shape=(self.n_agents, 2 + n_colors), dtype="uint8"
            )
...
class EmptyCoinGameEnv10x10Minigrid(CoinGameEnv):
    def __init__(self, **kwargs):
        super().__init__(size=10, n_agents=1, n_goals=2, n_colors=2, n_clutter=0, minigrid_mode=True, **kwargs)
