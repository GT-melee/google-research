...
class Agent(WorldObj):
    ...
    def render(self, img):
        ...
        tri_fn = rendering.rotate_fn(tri_fn, cx=0.5, cy=0.5, theta=0.5 * math.pi * self.dir)
...
class Grid(minigrid.Grid):
    ...
    @classmethod
    def render_tile(cls, obj, floor_color, highlight=None, tile_size=minigrid.TILE_PIXELS, subdivs=3, cell_type=None):
        ...
        for y in range(0, self.height):
            for x in range(0, self.width):
                ...
                tile_img = Grid.render_tile(
                    cell, self.floor_color, highlight=highlights, tile_size=tile_size, cell_type=cell_type,
                )
...
class MultiGridEnv(minigrid.MiniGridEnv):
    ...
    """
        ...
        self.direction_obs_space = gym.spaces.Box(low=0, high=3, shape=(self.n_agents,), dtype="uint8")
        ...
        if self.minigrid_mode:
            ...
            self.image_obs_space = gym.spaces.Box(low=0, high=255, shape=obs_image_shape, dtype="uint8")
        else:
            # First dimension of all observations is the agent ID
            self.action_space = gym.spaces.Box(low=0, high=len(self.actions) - 1, shape=(self.n_agents,), dtype="int64")
        ...
        # agent's direction
        observation_space = {"image": self.image_obs_space, "direction": self.direction_obs_space}
        if self.fully_observed:
    ...
    def reset(self):
        ...
        for a in range(self.n_agents):
            ...
            assert start_cell.type == "agent" or start_cell is None or start_cell.can_overlap()
    ...
    """
        ...
        for j in range(self.grid.height):
            for i in range(self.grid.width):
                ...
                agent_here = False
                for a in range(self.n_agents):
                    if self.agent_pos[a] is not None and (i == self.agent_pos[a][0] and j == self.agent_pos[a][1]):
    ...
    def place_one_agent(self, agent_id, top=None, size=None, rand_dir=True, max_tries=math.inf, agent_obj=None):
    ...
    def move_agent(self, agent_id, new_pos):
        ...
        assert (self.grid.get(new_pos[0], new_pos[1]).cur_pos == self.agent_pos[agent_id]).all()
    ...
    """
        ...
        grid.floor_color = (
            self.floor_color
        )  # must pass floor color through. This subgrid is the basic minigrid grid, that's kinda dumb
