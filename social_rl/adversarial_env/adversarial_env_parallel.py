...
class AdversarialParallelPyEnvironment(py_environment.PyEnvironment):
    ...
    def __init__(self, env_constructors, start_serially=True, blocking=False, flatten=False):
        """Batch together environments and simulate them in external processes.
        ...
        self._envs = [AdversarialProcessPyEnvironment(ctor, flatten=flatten) for ctor in env_constructors]
    ...
    def _step(self, actions):
        ...
        """
        time_steps = [
            env.step(action, self._blocking) for env, action in zip(self._envs, self._unstack_actions(actions))
        ]
    ...
    def get_num_blocks(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_array(tf.cast(self._envs[0].n_clutter_placed, tf.float32))
        else:
            return tf.stack([tf.cast(env.n_clutter_placed, tf.float32) for env in self._envs])

    def get_distance_to_goal(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_array(tf.cast(self._envs[0].distance_to_goal, tf.float32))
        else:
            return tf.stack([tf.cast(env.distance_to_goal, tf.float32) for env in self._envs])

    def get_deliberate_placement(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_array(tf.cast(self._envs[0].deliberate_agent_placement, tf.float32))
        else:
            return tf.stack([tf.cast(env.deliberate_agent_placement, tf.float32) for env in self._envs])

    def get_goal_x(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_array(tf.cast(self._envs[0].get_goal_x(), tf.float32))
        else:
            return tf.stack([tf.cast(env.get_goal_x(), tf.float32) for env in self._envs])

    def get_goal_y(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_array(tf.cast(self._envs[0].get_goal_y(), tf.float32))
        else:
            return tf.stack([tf.cast(env.get_goal_y(), tf.float32) for env in self._envs])

    def get_passable(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_array(tf.cast(self._envs[0].passable, tf.float32))
        else:
            return tf.stack([tf.cast(env.passable, tf.float32) for env in self._envs])

    def get_shortest_path_length(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_array(tf.cast(self._envs[0].shortest_path_length, tf.float32))
        else:
            return tf.stack([tf.cast(env.shortest_path_length, tf.float32) for env in self._envs])
    ...
    def _stack_time_steps(self, time_steps):
        ...
        if self._flatten:
            ...
            return nest_utils.fast_map_structure(lambda *arrays: np.stack(arrays), *time_steps)
    ...
    def _unstack_actions(self, batched_actions):
        ...
        if self._flatten:
            ...
            unstacked_actions = [
                tf.nest.pack_sequence_as(batched_actions, actions) for actions in zip(*flattened_actions)
            ]
