...
class AdversarialGymWrapper(gym_wrapper.GymWrapper):
    ...
    def __init__(
        ...
        super(AdversarialGymWrapper, self).__init__(
            gym_env, discount, spec_dtype_map, match_obs_space_dtype, auto_reset, simplify_box_bounds
        )
        ...
        self.adversary_time_step_spec = ts_lib.time_step_spec(self.adversary_observation_spec, self.reward_spec())
        self.adversary_flat_obs_spec = tf.nest.flatten(self.adversary_observation_spec)
    ...
    def reset_random(self):
        ...
        self._current_time_step = ts_lib.restart(observation, reward_spec=self.reward_spec())
    ...
    def _adversary_to_obs_space_dtype(self, observation):
        ...
        flat_obs = nest_utils.flatten_up_to(self.adversary_observation_spec, observation)

        matched_observations = []
        for spec, obs in zip(self.adversary_flat_obs_spec, flat_obs):
            matched_observations.append(np.asarray(obs, dtype=spec.dtype))
        return tf.nest.pack_sequence_as(self.adversary_observation_spec, matched_observations)
    ...
    def step_adversary(self, action):
        ...
        observation, reward, self._done, self._info = self._gym_env.step_adversary(action)
...
class AdversarialBatchedPyEnvironment(batched_py_environment.BatchedPyEnvironment):
    ...
    def __init__(self, envs, multithreading=True):
        super(AdversarialBatchedPyEnvironment, self).__init__(envs, multithreading=multithreading)
    ...
    def get_num_blocks(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_tensors(tf.cast(self._envs[0].n_clutter_placed, tf.float32))
        else:
            return tf.stack(lambda env: tf.cast(env.n_clutter_placed, tf.float32), self._envs)

    def get_distance_to_goal(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_tensors(tf.cast(self._envs[0].distance_to_goal, tf.float32))
        else:
            return tf.stack(lambda env: tf.cast(env.distance_to_goal, tf.float32), self._envs)

    def get_deliberate_placement(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_tensors(tf.cast(self._envs[0].deliberate_agent_placement, tf.float32))
        else:
            return tf.stack(lambda env: tf.cast(env.deliberate_agent_placement, tf.float32), self._envs)

    def get_goal_x(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_tensors(tf.cast(self._envs[0].get_goal_x(), tf.float32))
        else:
            return tf.stack(lambda env: tf.cast(env.get_goal_x(), tf.float32), self._envs)

    def get_goal_y(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_tensors(tf.cast(self._envs[0].get_goal_y(), tf.float32))
        else:
            return tf.stack(lambda env: tf.cast(env.get_goal_y(), tf.float32), self._envs)

    def get_passable(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_tensors(tf.cast(self._envs[0].passable, tf.float32))
        else:
            return tf.stack(lambda env: tf.cast(env.passable, tf.float32), self._envs)

    def get_shortest_path_length(self):
        if self._num_envs == 1:
            return nest_utils.batch_nested_tensors(tf.cast(self._envs[0].shortest_path_length, tf.float32))
        else:
            return tf.stack(lambda env: tf.cast(env.shortest_path_length, tf.float32), self._envs)
    ...
    def step_adversary(self, actions):
        if self._num_envs == 1:
            ...
            time_steps = self._execute(
                lambda env_action: env_action[0].step_adversary(env_action[1]), zip(self._envs, unstacked_actions)
            )
            return nest_utils.stack_nested_arrays(time_steps)
...
class AdversarialTFPyEnvironment(tf_py_environment.TFPyEnvironment):
    ...
    def __init__(self, environment, check_dims=False, isolation=False):
        ...
        super(AdversarialTFPyEnvironment, self).__init__(environment, check_dims=check_dims, isolation=isolation)

        if not environment.batched:
            self._env = AdversarialBatchedPyEnvironment([environment], multithreading=not self._pool)

        self._agent_time_step = None

        self.adversary_action_spec = tensor_spec.from_spec(self._env.adversary_action_spec)
        self.adversary_time_step_spec = tensor_spec.from_spec(self._env.adversary_time_step_spec)
        self.adversary_observation_spec = tensor_spec.from_spec(self._env.adversary_observation_spec)

        self._adversary_time_step_dtypes = [s.dtype for s in tf.nest.flatten(self.adversary_time_step_spec)]
    ...
    def reset_agent(self):
        ...
        with tf.name_scope("reset_agent"):
            reset_op = tf.numpy_function(_isolated_reset_py, [], [], name="reset_py_func")  # No inputs.
    ...
    def _current_agent_time_step(self):
        ...
        with tf.name_scope("current_agent_time_step"):
            ...
            flat_observations = outputs[3:]
            return self._set_names_and_shapes(
                self.time_step_spec(), self.observation_spec(), step_type, reward, discount, *flat_observations
            )
    ...
    def _set_names_and_shapes(self, ts_spec, obs_spec, step_type, reward, discount, *flat_observations):
    ...
    def _step(self, actions):
        def _step_py(*flattened_actions):
            with tf_py_environment._check_not_called_concurrently(self._lock):  # pylint:disable=protected-access
                packed = tf.nest.pack_sequence_as(structure=self.action_spec(), flat_sequence=flattened_actions)
        ...
        with tf.name_scope("step"):
            ...
            if self._check_dims:
                for action in flat_actions:
                    ...
                    if action.shape.rank == 0 or (dim_value is not None and dim_value != self.batch_size):
                        raise ValueError(
                            "Expected actions whose major dimension is batch_size (%d), "
                            "but saw action with shape %s:\n   %s" % (self.batch_size, action.shape, action)
                        )
            outputs = tf.numpy_function(_isolated_step_py, flat_actions, self._time_step_dtypes, name="step_py_func")
            step_type, reward, discount = outputs[0:3]
            flat_observations = outputs[3:]
    ...
    def step_adversary(self, actions):
        def _step_adversary_py(*flattened_actions):
            with tf_py_environment._check_not_called_concurrently(self._lock):  # pylint:disable=protected-access
                packed = tf.nest.pack_sequence_as(structure=self.adversary_action_spec, flat_sequence=flattened_actions)
