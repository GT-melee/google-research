...
class AdversarialDriver(object):
    ...
    def adversarial_episode(self):
        ...
        if self.adversary_agent:
            ...
            if self.flexible_protagonist:
                ...
                env_reward = protagonist_better * (agent_r_max - adv_agent_r_avg) + (1 - protagonist_better) * (
                    adv_agent_r_max - agent_r_avg
                )
        ...
        if self.debug:
            logging.info(
                "Agent reward: avg = %f, max = %f",
                tf.reduce_mean(agent_r_avg).numpy(),
                tf.reduce_mean(agent_r_max).numpy(),
            )
            logging.info("Environment score: %f", tf.reduce_mean(env_reward).numpy())
            if self.adversary_agent:
                logging.info(
                    "Adversary agent reward: avg = %f, max = %f",
    ...
    def combined_population_adversarial_episode(self):
        ...
        for agent_idx in range(len(self.agent)):
            agent_r_avg, agent_r_max, agent_idx_selected = self.run_agent(
                self.env, self.agent, self.env.reset_agent, self.env.step, agent_idx=agent_idx
            )
        ...
        adv_r = regret + self.compute_adversary_block_budget(population_max, env_idx)
        ...
        if self.debug:
            ...
            logging.info("Environment regret: %f", tf.reduce_mean(regret).numpy())
    ...
    def log_environment_metrics(self, agent_r_max):
        ...
        env_episodes = [tf.convert_to_tensor(self.total_episodes_collected, dtype=tf.float32)]
        ...
        for i, m in enumerate(
            [
                distance_to_goal,
                num_blocks,
                deliberate_placement,
                env_episodes,
                goal_x,
                goal_y,
                passable,
                shortest_path,
                shortest_passable_path,
                solved_path_length,
            ]
        ):
        ...
        if self.debug:
            ...
            logging.info("Distance to goal: %f", tf.reduce_mean(distance_to_goal).numpy())
            logging.info("Deliberate agent placement: %f", tf.reduce_mean(deliberate_placement).numpy())
            logging.info("Goal (X, Y): (%f, %f)", tf.reduce_mean(goal_x).numpy(), tf.reduce_mean(goal_y).numpy())
            logging.info("Possible to finish environment?: %f", tf.reduce_mean(passable).numpy())
            logging.info("Shortest path length to goal: %f", tf.reduce_mean(shortest_path).numpy())
            logging.info("Solved path length: %f", tf.reduce_mean(solved_path_length).numpy())
    ...
    def run_agent(self, env, agent_list, reset_func, step_func, agent_idx=None):
        ...
        while num_steps < agent.max_steps:
            ...
            if agent.name == "agent" and num_steps >= agent.max_steps - 1:
                outer_dims = nest_utils.get_outer_array_shape(next_time_step.reward, env.reward_spec())
                next_time_step = ts_lib.termination(
                    next_time_step.observation, next_time_step.reward, outer_dims=outer_dims
                )
    ...
    def compute_adversary_block_budget(self, antag_r_max, env_idx, use_shortest_path=True):
        ...
        block_budget_reward = antag_didnt_score * -weighted_budget + (1 - antag_didnt_score) * weighted_budget

        logging.info("Environment block budget reward: %f", tf.reduce_mean(block_budget_reward).numpy())
