...
flags.DEFINE_string(
    "root_dir", os.getenv("TEST_UNDECLARED_OUTPUTS_DIR"), "Root directory for writing logs/summaries/checkpoints."
)
flags.DEFINE_string("env_name", "MultiGrid-Adversarial-v0", "Name of an environment")
flags.DEFINE_integer("replay_buffer_capacity", 3001, "Replay buffer capacity per env.")
flags.DEFINE_integer("num_parallel_envs", 4, "Number of environments to run in parallel. Originally 30")
...
flags.DEFINE_integer("num_eval_episodes", 2, "The number of episodes to run eval on. Originally 30.")
flags.DEFINE_boolean("debug", False, "Turn on debugging and tf functions off.")
flags.DEFINE_boolean("agent_regret_off", False, "If True, agents will train with normal reward not regret")
flags.DEFINE_boolean(
    "unconstrained_adversary", False, "If True, does not use a second agent to constrain the " "adversary environment."
)
flags.DEFINE_boolean(
...
flags.DEFINE_boolean(
    "flexible_protagonist", False, "Which agent assumes the role of protagonist depends on " "the scores of both agents"
)
flags.DEFINE_integer("protagonist_episode_length", None, "Number of steps for protagonist episodes.")
flags.DEFINE_integer("adversary_population_size", 1, "Number of adversaries in the population")
flags.DEFINE_integer("protagonist_population_size", 1, "Number of training agents in the population")
flags.DEFINE_integer("antagonist_population_size", 1, "Number of agents aligned with the adversary")
flags.DEFINE_boolean(
    "combined_population",
    False,
...
flags.DEFINE_float("block_budget_weight", 0.0, "Coefficient used to impose block budget on the adversary.")
FLAGS = flags.FLAGS
    ...
    with tf.compat.v2.summary.record_if(lambda: tf.math.equal(global_step % summary_interval, 0)):
        ...
        for mname in env_metrics_names:
            env_train_metrics.append(
                adversarial_eval.AdversarialEnvironmentScalar(batch_size=num_parallel_envs, name=mname)
            )
            env_eval_metrics.append(
                adversarial_eval.AdversarialEnvironmentScalar(batch_size=num_eval_episodes, name=mname)
            )
            ...
            for i in range(population_size):
                ...
                agents[agent_name].append(
                    agent_train_package.AgentTrainPackage(
                        ...
                        max_steps=max_steps,
                        replace_reward=(not unconstrained_adversary and agents_learn_with_regret),
                        id_num=i,
        ...
        if global_step.numpy() == 0:
            config_filename = os.path.join(train_dir, "operative_config-{}.gin".format(global_step.numpy()))
        ...
        while global_step.numpy() <= num_train_steps:
            ...
            for name, agent_list in agents.items():
                ...
                for agent_idx in train_idxs[name]:
                    ...
                    if math.isnan(agent.total_loss) or math.isinf(agent.total_loss) or agent.total_loss > MAX_LOSS:
                ...
                if agent.is_environment:
                    agent.env_train_metric.tf_summaries(train_step=global_step, step_metrics=step_metrics)
                    ...
                    for name, agent_list in agents.items():
                        for i, agent in enumerate(agent_list):
                            ...
                            saved_model_path = os.path.join(
                                agent.saved_model_dir, "policy_" + ("%d" % global_step_val).zfill(9)
                            )
