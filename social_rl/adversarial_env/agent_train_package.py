...
class AgentTrainPackage(object):
    ...
    def __init__(
        ...
        with tf.name_scope(self.name):
            ...
            if is_environment:
                ...
                (self.actor_net, self.value_net) = multigrid_networks.construct_multigrid_networks(
            ...
            self.replay_buffer = tf_uniform_replay_buffer.TFUniformReplayBuffer(
                self.tf_agent.collect_data_spec, batch_size=num_parallel_envs, max_length=replay_buffer_capacity
            )
            ...
            self.train_metrics = step_metrics + [
                tf_metrics.AverageEpisodeLengthMetric(batch_size=num_parallel_envs, name=name + "_AverageEpisodeLength")
            ]
            self.eval_metrics = [
            ...
            if is_environment:  # TODO: modify here!
                ...
                self.eval_metrics.append(
                    tf_metrics.AverageReturnMetric(batch_size=num_eval_episodes, name=name + "_AverageReturn")
                )
            ...
            self.policy_checkpointer = common.Checkpointer(
                ckpt_dir=os.path.join(self.train_dir, "policy"), policy=self.eval_policy, global_step=global_step
            )
            self.saved_model = policy_saver.PolicySaver(self.eval_policy, train_step=global_step)
            self.saved_model_dir = os.path.join(root_dir, "policy_saved_model", name, str(id_num))
    ...
    def get_trajectories(self):
        ...
        if self.is_environment:
            ...
            replaced_reward = tf.sparse.SparseTensor(idx_remove, self.final_reward, trajectories.reward.shape)
            true_reward = tf.sparse.to_dense(replaced_reward, default_value=0.0)
        else:
            ep_len = trajectories.reward.shape[1]
            tiled_enemy_max = tf.tile(tf.reshape(self.enemy_max, (-1, 1)), (1, ep_len))
